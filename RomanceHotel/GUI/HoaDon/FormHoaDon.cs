...
namespace RomanceHotel.GUI
{
    public partial class FormHoaDon : Form
    {
        ...
        {
            // Gọi callback trước khi đóng
            OnHoaDonClosed?.Invoke();
        ...
        private void btnInReport_Click(object sender, EventArgs e)
        {
            try
            {
                List<HoaDonReportDTO> hdInfo = new List<HoaDonReportDTO>();
                List<HoaDonChiTietReportDTO> hdCT = new List<HoaDonChiTietReportDTO>();

                // --- LẤY THÔNG TIN HÓA ĐƠN ---
                hdInfo.Add(new HoaDonReportDTO()
                {
                    MaHD = this.TextBoxSoHD.Text,
                    TenKH = this.TextBoxTenKH.Text,
                    MaPhong = this.TextBoxMaPhong.Text,
                    TenNV = this.TextBoxTenNV.Text,
                    NgayHoaDon = this.TextBoxNgayHD.Text,
                    LoaiPhong = this.TextBoxLoaiPhong.Text,
                    SoNgay = this.TextBoxSoNgay.Text,
                    TriGia = this.money
                });
                ...
                foreach (DataGridViewRow row in DataGridViewDichVu.Rows)
                {
                    ...
                    hdCT.Add(new HoaDonChiTietReportDTO()
                    {
                        TenDV = row.Cells[0].Value.ToString(),
                        DonGia = row.Cells[1].Value.ToString(),
                        SoLuong = int.Parse(row.Cells[2].Value.ToString()),
                        ThanhTien = row.Cells[3].Value.ToString()
                    });
                }

                // --- MỞ FORM REPORT ---
                FormInHoaDon f = new FormInHoaDon(hdInfo, hdCT);
                f.ShowDialog();
            }
            catch (Exception ex)
            {
