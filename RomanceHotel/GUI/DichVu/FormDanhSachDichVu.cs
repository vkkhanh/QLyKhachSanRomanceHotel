    ...
    public partial class FormDanhSachDichVu : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Tạo list DTO
                List<DichVuReportDTO> dataReport = new List<DichVuReportDTO>();
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        dataReport.Add(new DichVuReportDTO
                        {
                            STT = stt++,
                            MaDV = row.Cells[1].Value?.ToString(),
                            TenDV = row.Cells[2].Value?.ToString(),
                            DonGia = decimal.Parse(row.Cells[3].Value?.ToString().Replace(",", "")),
                            SLConLai = Convert.ToInt32(row.Cells[4].Value?.ToString())
                        });
                    }
                }

                // Gán vào Report
                rptDanhSachDichVu rpt = new rptDanhSachDichVu();
