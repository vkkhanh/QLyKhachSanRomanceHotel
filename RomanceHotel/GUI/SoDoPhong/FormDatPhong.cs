...
namespace RomanceHotel.GUI
{
    public partial class FormDatPhong : Form
    {
        ...
        private void LoadTenKH()
        {
            try
            {
                if (this.phieuThue != null)
                {
                    ...
                    CTTextBoxNhapEmail.RemovePlaceholder(); // THÊM
                    ...
                    this.CTTextBoxNhapEmail.Texts = khachHang.Email; // THÊM
                }
            }
            catch (Exception ex)
            {
        ...
        private void CTButtonDatTruoc_Click(object sender, EventArgs e)
        {
            ...
            Regex regexSDT = new Regex(patternSDT);
                ...
                this.ComboBoxGioiTinh.Texts != "  Giới tính")
            {
                ...
                // Kiểm tra SĐT
                if (!regexSDT.IsMatch(CTTextBoxNhapSDT.Texts))
                {
                ...
                // Kiểm tra Email
                if (!regexEmail.IsMatch(CTTextBoxNhapEmail.Texts))
                {
        ...
        private void CreateKH()
        {
            try
            {
                if (phieuThue == null)
                {
                    ...
                    khachHang.Email = this.CTTextBoxNhapEmail.Texts; // THÊM
        ...
        private void CTTextBoxNhapCCCD__TextChanged(object sender, EventArgs e)
        {
            ...
            if (caseForm == 0)
            {
                if (KhachHangBUS.Instance.FindKHWithCCCD(textBox.Text) != null)
                {
                    ...
                    CTTextBoxNhapEmail.Texts = khachHang.Email; // THÊM
