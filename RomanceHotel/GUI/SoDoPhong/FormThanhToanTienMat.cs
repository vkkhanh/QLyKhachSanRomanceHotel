...
namespace RomanceHotel.GUI
{
    public partial class FormThanhToanTienMat : Form
    {
        ...
        // 👉 Callback báo cho form ngoài biết đã thanh toán xong
        public Action OnPaymentCompleted { get; set; }

        public FormThanhToanTienMat(decimal tongTien, CTDP ctdp, TaiKhoan tk, Phong phong)
        {
            InitializeComponent();
        ...
        {
            decimal dua;
            if (decimal.TryParse(txtTienKhachDua.Text, out dua))
            {
                decimal thoi = dua - TongTien;
                lblTienThoi.Text = thoi >= 0 ? thoi.ToString("#,# VNĐ") : "0 VNĐ";
            }
            else
            {
                lblTienThoi.Text = "0 VNĐ";
            }
        }
