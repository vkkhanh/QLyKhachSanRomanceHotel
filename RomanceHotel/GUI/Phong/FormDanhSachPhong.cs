    ...
    public partial class FormDanhSachPhong : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<PhongReportDTO> dataReport = new List<PhongReportDTO>();

                int stt = 1;

                foreach (var phong in phongs)
                {
                    dataReport.Add(new PhongReportDTO
                    {
                        STT = stt++,
                        MaPH = phong.MaPH,
                        TenLPH = phong.LoaiPhong != null ? phong.LoaiPhong.TenLPH : string.Empty,
                        TTPH = phong.TTPH,
                        TTDD = phong.TTDD,
                        GhiChu = phong.GhiChu   // ← LẤY GHI CHÚ ĐÚNG TỪ DB
                    });
                }

                rptDanhSachPhong rpt = new rptDanhSachPhong();
