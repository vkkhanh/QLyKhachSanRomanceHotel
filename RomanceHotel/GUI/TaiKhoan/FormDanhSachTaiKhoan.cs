    ...
    public partial class FormDanhSachTaiKhoan : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<TaiKhoanReportDTO> dataReport = new List<TaiKhoanReportDTO>();
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        dataReport.Add(new TaiKhoanReportDTO
                        {
                            STT = stt++,
                            TenTK = row.Cells[1].Value?.ToString(),
                            TenNhanVien = row.Cells[2].Value?.ToString(),
                            CapDoQuyen = Convert.ToInt32(row.Cells[3].Value)
                        });
                    }
                }

                // Load Crystal Report
                rptDanhSachTaiKhoan rpt = new rptDanhSachTaiKhoan();
