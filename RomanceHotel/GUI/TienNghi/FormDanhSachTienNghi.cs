    ...
    public partial class FormDanhSachTienNghi : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<TienNghiReportDTO> dataReport = new List<TienNghiReportDTO>();
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        dataReport.Add(new TienNghiReportDTO
                        {
                            STT = stt++,
                            MaTN = row.Cells[1].Value?.ToString(),
                            TenTN = row.Cells[2].Value?.ToString()
                        });
                    }
                }

                rptDanhSachTienNghi rpt = new rptDanhSachTienNghi();
