using RomanceHotel.ApplicationSettings;
...
namespace RomanceHotel.GUI
{
    public partial class FormKetNoiCSDL : Form
    {
        ...
        {
            #region Check DB Connection 
            string connectionString = @"data source=(LocalDB)\MSSQLLocalDB;attachdbfilename=|DataDirectory|\Database\RomanceHotel.mdf;integrated security=True;connect timeout=30;MultipleActiveResultSets=True;App=EntityFramework";
            try
            {
                SqlHelper sqlHelper = new SqlHelper(connectionString);
                if (sqlHelper.isConnected)
