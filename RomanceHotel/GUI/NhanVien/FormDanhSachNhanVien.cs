    ...
    public partial class FormDanhSachNhanVien : Form
    {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<NhanVienReportDTO> dataReport = new List<NhanVienReportDTO>();
                ...
                foreach (DataGridViewRow row in grid.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        dataReport.Add(new NhanVienReportDTO
                        {
                            STT = stt++,
                            MaNV = row.Cells[1].Value?.ToString(),
                            TenNV = row.Cells[2].Value?.ToString(),
                            ChucVu = row.Cells[3].Value?.ToString(),
                            NgaySinh = row.Cells[4].Value?.ToString(),
                            GioiTinh = row.Cells[5].Value?.ToString(),
